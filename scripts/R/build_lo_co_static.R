...
for (lang in c("en", "es")) {
  ...
  plot_dir = paste0('public_html/', lang, 'img/lower-co-map')
  ...
  svg <- xmlParse(svg_file, useInternalNode=TRUE) %>%
    ...
    add_text(text=tr(lowCoFigLowerBasin, lang), x="320", y="230", 'fill'='#B22C2C', 'text-anchor'="start") %>%
    add_text(text=tr(lowCoFigUpperBasin, lang), x="338", y="130", 'fill'='#B22C2C', 'text-anchor'="start") %>%
    add_text(text=tr(lowCoFigRiver, lang), x="345", y="70", 'fill'='#1975d1', 'text-anchor'="start") %>%
    toString.XMLNode()
